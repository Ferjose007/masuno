...
<body class="bg-gray-50 min-h-screen flex flex-col md:flex-row">
    ...
    <main class="flex-1 w-full ml-0 md:ml-64 transition-all duration-300">
        ...
        <button id="openSidebar"
            class="md:hidden fixed top-1/2 left-0 z-40 transform -translate-y-1/2 bg-indigo-600 text-white p-3 pr-4 rounded-r-2xl shadow-lg opacity-50 hover:opacity-100 transition-all duration-300 focus:outline-none ">
        ...
        <div class="p-4 md:p-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                ...
                <button onclick="openModal('create')" class="inline-flex items-center justify-center bg-indigo-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-indigo-700 transition shadow-sm cursor-pointer">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0c0 .884.896 1.688 2 2.308 1.104-.62 2-1.424 2-2.308" /></svg>
                    Nuevo Estilista
                </button>
            </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[600px]">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($estilistas)): ?>
                                    <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400">No hay estilistas registrados.</td></tr>
                            <?php else: ?>
                                    <?php foreach ($estilistas as $e): ?>
                                            <tr class="hover:bg-gray-50 transition-colors <?= ($e->activo == 0) ? 'bg-gray-50 opacity-75' : '' ?>">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center">
                                                        <div class="h-8 w-8 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-sm font-bold mr-3 overflow-hidden">
                                                            <?php if (!empty($e->foto)): ?>
                                                                    <img src="<?= BASE_URL ?>/uploads/users/<?= $e->foto ?>" class="w-full h-full object-cover">
                                                            <?php else: ?>
                                                ...
                                                <td class="px-6 py-4 text-right space-x-2">
                                                    <button onclick='openViewModal(<?= json_encode($e) ?>)' class="text-blue-600 hover:text-blue-900 cursor-pointer" title="Ver Detalles"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                                                    <button onclick='openModal("edit", <?= json_encode($e) ?>)' class="text-indigo-600 hover:text-indigo-900 cursor-pointer" title="Editar"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                                    <?php if ($e->activo == 1): ?>
                                                            <button onclick="openToggleModal(<?= $e->id ?>, 'desactivar', '<?= htmlspecialchars($e->nombre) ?>')" class="text-amber-600 hover:text-amber-900 cursor-pointer" title="Desactivar"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg></button>
                                                    <?php else: ?>
                                                            <button onclick="openToggleModal(<?= $e->id ?>, 'activar', '<?= htmlspecialchars($e->nombre) ?>')" class="text-green-600 hover:text-green-900 cursor-pointer" title="Reactivar"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                                                    <?php endif; ?>
                                                    <button onclick="openDeleteModal(<?= $e->id ?>)" class="text-red-600 hover:text-red-900 cursor-pointer" title="Eliminar"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                                </td>
                                            </tr>
                                    <?php endforeach; ?>
                            <?php endif; ?>
    ...
    <script>
        ...
        function openViewModal(data) {
            ...
            if (data.foto && data.foto.trim() !== "") {
                ...
                avatarDiv.classList.add('bg-purple-100', 'text-purple-600');
            }
            ...
            if (data.lista_servicios && data.lista_servicios.length > 0) {
                data.lista_servicios.forEach(s => {
                    let clases = 'bg-indigo-50 text-indigo-700 border border-indigo-100';
                    if (s.activo == 0) clases = 'bg-red-50 text-red-500 border border-red-100 line-through opacity-75';
                    listaDiv.innerHTML += `<span class="px-3 py-1 rounded-full text-xs font-medium ${clases}">${s.nombre}</span>`;
                });
            } else {
                listaDiv.innerHTML = '<span class="text-gray-400 text-sm italic">Ningún servicio asignado</span>';
            }
        ...
        // --- TOGGLE & DELETE (Sin cambios importantes) ---
        let toggleId = null;
        function openToggleModal(id, action, nombre) {
            ...
            if (action === 'desactivar') {
                ...
                msg.textContent = "El estilista perderá el acceso.";
                ...
                msg.textContent = "El estilista podrá ingresar.";
        ...
        function closeToggleModal() { document.getElementById('toggle-modal').classList.add('hidden'); toggleId = null; }
