...
<body class="bg-gray-50 min-h-screen">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php if(empty($clientes)): ?>
                    <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400">No hay clientes registrados.</td></tr>
                <?php else: ?>
                    <?php foreach ($clientes as $c): ?>
                    <tr class="hover:bg-gray-50 transition-colors <?= ($c->activo == 0) ? 'bg-gray-50 opacity-75' : '' ?>">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="h-8 w-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-sm font-bold mr-3">
                                    <?= strtoupper(substr($c->nombre, 0, 1)) ?>
                                </div>
                                <div>
                                    <span class="font-medium text-gray-800 block"><?= htmlspecialchars($c->nombre) ?></span>
                                    <?php if($c->activo == 0): ?>
                        ...
                        <td class="px-6 py-4 text-gray-600 text-sm"><?= htmlspecialchars($c->telefono ?? '-') ?></td>
                        <td class="px-6 py-4 text-right space-x-2">
                            ...
                            <?php if($c->activo == 1): ?>
                                <button onclick="openToggleModal(<?= $c->id ?>, 'desactivar', '<?= htmlspecialchars($c->nombre) ?>')"
                                    class="text-amber-600 hover:text-amber-900 font-medium text-sm transition-colors cursor-pointer" title="Anular (Desactivar)">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                                    </svg>
                                </button>
                            <?php else: ?>
                                <button onclick="openToggleModal(<?= $c->id ?>, 'activar', '<?= htmlspecialchars($c->nombre) ?>')"
                                    class="text-green-600 hover:text-green-900 font-medium text-sm transition-colors cursor-pointer" title="Reactivar">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  <div id="view-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50 backdrop-blur-sm transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4 relative transform scale-100">
        ...
        <div class="text-center mb-6">
            <div class="h-20 w-20 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 text-3xl font-bold mx-auto mb-3" id="view-avatar">
                A
            </div>
            <h3 class="text-xl font-bold text-gray-900" id="view-nombre">Nombre Cliente</h3>
        ...
        <div class="space-y-4 border-t border-gray-100 pt-4">
            <div class="flex justify-between">
                ...
                <span class="text-gray-800 font-medium text-sm" id="view-email">-</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
                <span class="text-gray-500 text-sm">Registrado el:</span>
                <span class="text-gray-800 font-medium text-sm" id="view-creado">-</span>
            </div>
            <div class="flex justify-between">
                <span class="text-gray-500 text-sm">Última modificación:</span>
                <span class="text-gray-800 font-medium text-sm" id="view-actualizado">-</span>
            </div>
        </div>
        
        <div class="mt-6 pt-4 border-t border-gray-100 text-center">
            <button onclick="closeViewModal()" class="w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">Cerrar</button>
        </div>
    </div>
  </div>
  ...
  <div id="form-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50 backdrop-blur-sm transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all scale-100 mx-4">
        ...
        <form id="client-form" action="" method="post" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nombre" id="client-nombre" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email <span class="text-red-500">*</span></label>
                    <input type="email" name="email" id="client-email" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
                    <input type="text" name="telefono" id="client-telefono" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" id="label-password">Contraseña</label>
                <input type="password" name="password" id="client-password" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500" placeholder="••••••••">
                <p class="text-xs text-gray-500 mt-1 hidden" id="hint-password">Dejar en blanco para mantener la actual.</p>
            </div>
  ...
  <script>
    ...
    function openViewModal(data) {
        ...
        // Fechas
        const fechaCreacion = data.creado_en ? new Date(data.creado_en).toLocaleDateString('es-PE', {
        ...
        if (data.actualizado_en) {
            fechaMod = new Date(data.actualizado_en).toLocaleDateString('es-PE', {
    ...
    function openToggleModal(id, action, nombre) {
        ...
        if (action === 'desactivar') {
            ...
            msg.textContent = "El usuario no podrá iniciar sesión, pero sus datos se conservarán.";
            ...
            msg.textContent = "El usuario podrá volver a iniciar sesión inmediatamente.";
